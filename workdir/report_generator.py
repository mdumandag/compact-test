...
def generate_report(test_name, sizes, kinds):
    for size in sizes:
        for test_type in TEST_TYPES:
            folders = [
                f"{test_name}_{kind}_{size}_{test_type}" for kind in kinds
            ]

            subprocess.run(["benchmark-report", "-o", f"reports/{test_name}_{size}_{test_type}", *folders])


generate_report("write", OBJECT_SIZES, OBJECT_KINDS)
generate_report("read", OBJECT_SIZES, OBJECT_KINDS)
generate_report("query", OBJECT_SIZES[:-1], OBJECT_KINDS[:-1])
